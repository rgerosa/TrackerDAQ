...
TrackerDpgAnalysis::TrackerDpgAnalysis(const edm::ParameterSet& iConfig):hltConfig_()
{
   ...
   pixelVertexToken_  = mayConsume<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("pixelVertexLabel"));
   ...
   pixclusters_->Branch("lumiSegment",&lumiSegment_,"lumiSegment/s");
   for(size_t i = 0; i<trackSize; ++i) {
     char buffer1[256];
     char buffer2[256];
     sprintf(buffer1,"trackid%lu",(unsigned long)i);
   ...
   for(size_t i = 0; i<trackSize; ++i) {
     ...
     themissingHits_->Branch("lumiSegment",&lumiSegment_,"lumiSegment/s");
   ...
   pixelVertices_->Branch("lumiSegment",&lumiSegment_,"lumiSegment/s");
...
TrackerDpgAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   // Pixel vertices are handled as primary vertices, but not linked to tracks.
    ...
    if(pixelVertexCollectionHandle.isValid())
      nPixelVertices_ = pixelVertexCollectionHandle->size();
   ...
   if(functionality_pixvertices_ and pixelVertexCollectionHandle.isValid()) {
     for(reco::VertexCollection::const_iterator v = pixelVertexCollectionHandle->begin();
         v!=pixelVertexCollectionHandle->end(); ++v) {
       nTracks_pvtx_ = v->tracksSize();
       sumptsq_pvtx_ = sumPtSquared(*v);
       isValid_pvtx_ = int(v->isValid());
       isFake_pvtx_ =  int(v->isFake());
...
TrackerDpgAnalysis::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup)
{
   ...
   if(not delayMap.empty()){
     TString mapname (delayFileNames_.front());
     ...
     tmap.save(true,-10,10,std::string(mapname+".png"),1400,800);
   }
